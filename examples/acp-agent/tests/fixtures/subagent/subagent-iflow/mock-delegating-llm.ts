 ...
 * verbatim — so the fake iFlow child's answer reaches the REPL stdout for the
 * driving e2e to assert.
 */
class MockDelegatingAdapter extends LlmAdapter {
  async * stream(options: GenerateOptions): AsyncIterable<StreamChunk> {
    ...
    if (toolResultText.length === 0) {
      const args = JSON.stringify({ description: 'iflow probe', prompt: 'report your answer' })
